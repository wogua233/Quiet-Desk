...
{
    internal static int Run(string[] args){if(args.Contains("--publisher-live"))return PublisherApiVerification.Live(args[Array.IndexOf(args,"--out")+1]).GetAwaiter().GetResult();if(args.Contains("--short-check"))return ShortCheck(args).GetAwaiter().GetResult();if(args.Contains("--layout"))return Layout(args);if(args.Contains("--live"))return Live(args).GetAwaiter().GetResult();int oi=Array.IndexOf(args,"--out");string output=oi<0?"reading-test.txt":args[oi+1];var lines=new System.Collections.Generic.List<string>();void Check(bool ok,string name){if(!ok)throw new Exception(name);lines.Add("PASS "+name);}try{
        ...
        PaginationVerification.Checks(Path.Combine(dir,"pagination-tests"),lines);
        SourceVerification.Checks(Path.Combine(dir,"source-tests"),lines).GetAwaiter().GetResult();
        PublisherApiVerification.Checks(Path.Combine(dir,"publisher-tests"),lines).GetAwaiter().GetResult();
