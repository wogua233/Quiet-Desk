...
Beta 安装包采用实心 LZMA 压缩并只携带简体中文卫星资源，以缩小传播体积。当前安装包约 260 MiB，上一构建约 339 MiB，减少约 23%。安装后的核心程序仍是完整的 Windows x64 自包含版本；离线声音、FFmpeg 解码能力和用户数据格式没有删减。实心压缩会让安装准备阶段比旧包稍慢，并在安装时使用约 64 MB 的解压字典内存。
